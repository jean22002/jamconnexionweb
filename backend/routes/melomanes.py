...
def _normalize_melomane(doc):
    """Sanitize legacy melomane records before Pydantic validation."""
    if not doc:
        return doc
    # Coerce datetime → ISO string for legacy records that stored python datetime
...
async def get_all_melomanes():
    try:
        ...
        return [MelomaneResponse(**_normalize_melomane(m)) for m in melomanes]
...
async def get_melomane_by_id(melomane_id: str):
    ...
    return MelomaneResponse(**_normalize_melomane(melomane))
