...
# ============================================================================
# CANCELLATION VALIDATION FLOW (Build 152.6 — Juin 2026)
# Musicien demande annulation d'une candidature 'accepted' → bar valide/refuse.
# ============================================================================
...
async def cancel_application_v2(
    ...
    if status == "accepted":
        ...
        # Notifier le bar propriétaire du créneau
        slot = await db.planning_slots.find_one({"id": app["planning_slot_id"]}, {"_id": 0})
        if slot:
            venue = await db.venues.find_one({"id": slot["venue_id"]}, {"_id": 0})
            if venue and venue.get("user_id"):
...
async def validate_cancellation(
    ...
    - approve=True  → status='cancelled', cancellation_status='approved', slot ré-ouvert si besoin
    - approve=False → status reste 'accepted', cancellation_status='refused'
    """
    if current_user["role"] != "venue":
    ...
    # Vérifier que le venue est bien propriétaire du slot
    ...
    if payload.approve:
        # Approve → candidature cancelled + slot potentiellement ré-ouvert
        await db.applications.update_one(
            {"id": app_id},
            {"$set": {
                "status": "cancelled",
        ...
        # Ré-ouvrir le slot si nécessaire (comme delete_application)
        accepted_count = await db.applications.count_documents({
            "planning_slot_id": slot["id"],
            "status": "accepted",
        })
...
async def list_pending_cancellations(current_user: dict = Depends(get_current_user)):
    ...
    Utilisé pour afficher un badge de notification côté venue.
    """
    if current_user["role"] != "venue":
    ...
    # Récupère tous les slots de ce venue
    slot_ids = [
        s["id"]
        for s in await db.planning_slots.find(
            {"venue_id": venue["id"]}, {"_id": 0, "id": 1}
    ...
    # Enrichir avec infos slot + musicien pour l'UI
    enriched = []
    for a in apps:
        slot = await db.planning_slots.find_one(
