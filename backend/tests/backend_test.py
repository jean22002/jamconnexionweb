...
class JamConnexionAPITester:
    def __init__(self, base_url="https://moderation-dashboard-1.preview.emergentagent.com/api"):
    ...
    def test_messaging_restriction_everyone_allowed(self):
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/auth/register", json=test_data, timeout=10)
            if response.status_code != 200:
                self.log_test("Messaging Restriction - Everyone Allowed", False, "Failed to create venue")
                return False
                
            venue_data = response.json()
            self.msg_venue_token = venue_data.get('token')
            self.msg_venue_user = venue_data.get('user')
            ...
            headers = {'Authorization': f'Bearer {self.msg_venue_token}'}
            response = requests.post(f"{self.base_url}/venues", json=venue_profile_data, headers=headers, timeout=10)
            
            if response.status_code == 200:
                venue_profile = response.json()
                self.msg_venue_profile_id = venue_profile.get('id')
                
                # Now test that musician can send message
                message_data = {
                    "recipient_id": self.msg_venue_user['id'],
    ...
    def test_messaging_restriction_connected_only_blocked(self):
        ...
        try:
            ...
            if response.status_code == 200:
                # Now test that same musician CANNOT send message
                message_data = {
                    "recipient_id": self.msg_venue_user['id'],
    ...
    def test_messaging_restriction_with_accepted_application(self):
        ...
        try:
            ...
            venue_headers = {'Authorization': f'Bearer {self.msg_venue_token}'}
            response = requests.post(f"{self.base_url}/planning", json=planning_data, headers=venue_headers, timeout=10)
            
            if response.status_code == 200:
                planning_response = response.json()
                self.msg_planning_slot_id = planning_response.get('id')
                
                # Create application from musician
                application_data = {
                    "planning_slot_id": self.msg_planning_slot_id,
                ...
                if response.status_code == 200:
                    app_response = response.json()
                    self.msg_application_id = app_response.get('id')
                    
                    # Accept the application
                    response = requests.post(f"{self.base_url}/applications/{self.msg_application_id}/accept", headers=venue_headers, timeout=10)
                    
                    if response.status_code == 200:
                        # Now test that musician CAN send message (has accepted application)
                        message_data = {
                            "recipient_id": self.msg_venue_user['id'],
                        ...
                        success = False
                        details = f"Failed to accept application: {response.status_code}"
                else:
                    success = False
                    details = f"Failed to create application: {response.status_code}"
            else:
                success = False
                details = f"Failed to create planning slot: {response.status_code}"
    ...
    def test_messaging_restriction_second_venue_isolation(self):
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/auth/register", json=test_data, timeout=10)
            if response.status_code != 200:
                self.log_test("Messaging Restriction - Second Venue Isolation", False, "Failed to create second venue")
                return False
                
            venue2_data = response.json()
            self.msg_venue2_token = venue2_data.get('token')
            self.msg_venue2_user = venue2_data.get('user')
            ...
            headers2 = {'Authorization': f'Bearer {self.msg_venue2_token}'}
            ...
            self.log_test("Messaging Restriction - Second Venue Isolation", success, details)
            return success
        except Exception as e:
    ...
    def run_all_tests(self):
        ...
        tests = [
            ...
            # NOTIFICATIONS SYSTEM COMPREHENSIVE TESTS (Review Request)
            self.test_notifications_authentication_required,
            self.test_notifications_api_get_all,
            self.test_notifications_api_unread_count,
            self.test_notifications_api_mark_read,
            self.test_notifications_api_mark_all_read,
            self.test_notifications_api_delete,
            self.test_notifications_user_filtering,
            self.test_create_test_events_for_notifications,
            self.test_notifications_script_execution,
            self.test_verify_notifications_created,
            self.test_notifications_daemon_status,
            self.test_notifications_daemon_logs,
    ...
    def test_stripe_checkout_creation_venue(self):
        ...
        try:
            headers = {'Authorization': f'Bearer {self.venue_token}'}
            checkout_data = {
                "origin_url": "https://moderation-dashboard-1.preview.emergentagent.com"
            }
