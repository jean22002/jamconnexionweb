...
import { FileText, TrendingUp, DollarSign, Calendar, Filter, Download, Loader2, Check, X, Clock, Eye, Edit3 } from 'lucide-react';
...
import MusicianAccountingEditModal from './MusicianAccountingEditModal';
...
const AccountingTab = ({ token }) => {
  ...
  const fetchMusicianEntries = async () => {
    try {
      const list = await musicianAccountingApi.list(token);
      const map = {};
      for (const entry of list) {
        if (entry?.event_id) map[String(entry.event_id)] = entry;
      }
      setEntriesByEventId(map);
    } catch (error) {
      console.error('Error fetching musician accounting entries:', error);
    }
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      await Promise.all([fetchSummary(), fetchConcerts(), fetchMusicianEntries()]);
      setLoading(false);
    };
    loadData();
...
const GeneralAccountingContent = ({ 
  ...
  return (
    <div className="space-y-6">
        ...
        {filteredConcerts.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredConcerts.map((concert) => {
              const entry = entriesByEventId[String(concert.id)] || null;
              return (
              <div key={concert.id} className="bg-white/5 border border-white/10 rounded-xl p-5 hover:bg-white/10 transition-all">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {entry?.payment_method && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-primary/15 text-primary border border-primary/30 capitalize">
                            {entry.is_guso ? '🎵 GUSO' : entry.payment_method}
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    {entry && (entry.amount != null || entry.cachet_net != null) && (
                      <div className="mt-3 grid grid-cols-3 gap-2 p-3 rounded-lg bg-black/20 border border-white/5">
                        <div className="text-center">
                          <p className="text-[10px] uppercase tracking-wide text-muted-foreground">Brut</p>
                          <p className="text-sm font-bold">{(entry.amount ?? 0).toFixed(2)}€</p>
                        </div>
                        <div className="text-center">
                          <p className="text-[10px] uppercase tracking-wide text-muted-foreground">Charges</p>
                          <p className="text-sm font-bold text-orange-400">
                            {entry.charges_sociales != null ? `-${entry.charges_sociales.toFixed(2)}€` : '—'}
                          </p>
                        </div>
                        <div className="text-center">
                          <p className="text-[10px] uppercase tracking-wide text-muted-foreground">Net</p>
                          <p className="text-sm font-bold text-green-400">
                            {entry.cachet_net != null ? `${entry.cachet_net.toFixed(2)}€` : '—'}
                          </p>
                        </div>
                      </div>
                    )}

                    {(entry?.notes || concert.notes) && (
                      <p className="text-xs text-muted-foreground mt-2 italic">Note: {entry?.notes || concert.notes}</p>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-3">
                    ...
                    <Button
                      ...
                      onClick={() => setEditingConcert({ ...concert, type: 'concert' })}
                      data-testid={`musician-accounting-edit-btn-${concert.id}`}
                    >
      ...
      {/* Modale d'édition Comptabilité Musicien */}
      <MusicianAccountingEditModal
        open={!!editingConcert}
        onOpenChange={(open) => !open && setEditingConcert(null)}
        token={token}
        event={editingConcert}
        initialEntry={editingConcert ? entriesByEventId[String(editingConcert.id)] : null}
        onSaved={(saved) => {
          setEntriesByEventId((prev) => ({ ...prev, [String(saved.event_id)]: saved }));
        }}
        onDeleted={(eventId) => {
