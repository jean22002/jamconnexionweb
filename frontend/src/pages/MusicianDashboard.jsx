...
export default function MusicianDashboard() {
  ...
  // Ad interstitial pour les non-PRO avant d'envoyer un message à un groupe
  ...
  // Envoi effectif du message de contact à un groupe (appelé après validation
  // de la pub pour les non-PRO, ou directement pour les PRO)
  const sendBandContactMessage = async () => {
    try {
      await axios.post(
  ...
  const handleDeleteBand = async (index) => {
    const bandToDelete = profileForm.bands[index];
    ...
    try {
      ...
      await axios.put(`${API}/musicians`, profileData, { headers: { Authorization: `Bearer ${token}` } });
      toast.success(`Groupe "${bandToDelete.name}" supprimé`);
    } catch (error) {
      // Restore on failure
  ...
  const cancelApplication = async (appId) => {
    ...
    try {
      // Build 152.11 — Utilise le nouveau endpoint (pending → delete / accepted → cancellation_requested)
  ...
  return (
    <div className="min-h-screen bg-background" data-testid="musician-dashboard">
                      ...
                      <div className="space-y-3">
                        ...
                        {currentBand.looking_for_members && (
                          <div className="space-y-2 pl-6 border-l-2 border-primary/30">
                            <Label className="text-sm">Profils recherchés <span className="text-muted-foreground text-xs">({currentBand.looking_for_profiles?.length || 0} sélectionné{(currentBand.looking_for_profiles?.length || 0) > 1 ? 's' : ''})</span></Label>
                              ...
                              {BAND_MEMBER_PROFILES.map((profile) => (
                                <div key={profile} className="flex items-center space-x-2">
                                  <input
                                    ...
                                    className="w-4 h-4 text-primary bg-black/20 border-white/10 rounded focus:ring-primary focus:ring-2"
                                    data-testid={`profile-checkbox-${profile}`}
                                  />
                                  <Label 
      ...
      {/* Pub interstitielle pour les musiciens NON-PRO avant envoi du message */}
      <AdInterstitial
        open={showAdInterstitial}
        onClose={() => setShowAdInterstitial(false)}
        onContinue={() => sendBandContactMessage()}
      />
    </div>
  );
}
